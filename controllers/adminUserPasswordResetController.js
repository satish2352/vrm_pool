                ...
                if (user_type) {
                    if (user_type == '1') {
                        ...
                        if (req.body.mobile == user.mobile) {                                                                          
                            ...
                            await user.save().then(async updatedUser=>{
                                try {
                                    await transporter.sendMail({
                                        ...
                                        to: user.email,
                                        subject: 'Password Is Reset For Your Account',
                                        text: `Dear ${user.name},\nWelcome to our platform! Your password has been reset use following password to login. ${randomPassword}`,
                                    });
                            ...
                            return res.status(200).send({ result: true, message: "User Password Changed Successfully" });
                        }
                        else {
